...
class Setting_model extends CI_Model {
    ...
    public function get($id = null) {
        ...
        $this->db->select('sch_settings.id,sch_settings.lang_id,sch_settings.class_teacher,sch_settings.is_rtl,sch_settings.cron_secret_key, sch_settings.timezone,
          sch_settings.name,sch_settings.email,sch_settings.phone,languages.language, sch_settings.previous_session_balance_tax,
          sch_settings.address,sch_settings.dise_code,sch_settings.date_format,sch_settings.currency,sch_settings.currency_symbol,sch_settings.start_month,sch_settings.session_id,sch_settings.fee_due_days,sch_settings.fee_tax,sch_settings.sales_tax, sch_settings.image,sch_settings.theme,sessions.session'
        );
    ...
    public function getSalesTax(){
           $this->db->select('sch_settings.sales_tax');
        $this->db->from('sch_settings');
        $query = $this->db->get();
        $result = $query->result_array();
        return $result[0];
    ...
    public function getFeeTax(){
        $this->db->select('sch_settings.fee_tax');
        $this->db->from('sch_settings');
        $query = $this->db->get();
        $result = $query->result_array();
        return $result[0];
	...
	public function getPreviousSessionBalanceTax(){
        $this->db->select('sch_settings.previous_session_balance_tax');
        $this->db->from('sch_settings');
        $query = $this->db->get();
        $result = $query->result_array();
        return $result[0];
    ...
    public function getSetting() {
        ...
        $this->db->select('sch_settings.id,sch_settings.lang_id,sch_settings.is_rtl,sch_settings.fee_due_days,sch_settings.fee_tax,sch_settings.sales_tax, sch_settings.fee_tax,sch_settings.class_teacher,sch_settings.cron_secret_key,sch_settings.timezone,
          sch_settings.name,sch_settings.email,sch_settings.phone,languages.language, 
          sch_settings.address,sch_settings.dise_code,sch_settings.date_format,sch_settings.currency,sch_settings.currency_symbol,sch_settings.start_month,sch_settings.session_id,sch_settings.image,sch_settings.theme,sessions.session,sch_settings.previous_session_balance_tax'
        );
        $this->db->from('sch_settings');
        $this->db->join('sessions', 'sessions.id = sch_settings.session_id');
        $this->db->join('languages', 'languages.id = sch_settings.lang_id');
