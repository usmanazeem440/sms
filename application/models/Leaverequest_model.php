...
class Leaverequest_model extends CI_model {
    ...
    public function leaveType($id) {
        ...
        $query = $this->db->select('staff_leave_details.*,leave_types.type,leave_types.id as typeid')
                        ->where(array('staff_id' => $id))
                        ->join("leave_types", "staff_leave_details.leave_type_id = leave_types.id","left")
                        ->group_by("leave_types.id")
