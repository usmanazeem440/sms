...
class Parents extends Parent_Controller {
    ...
    function getAllFees($parentId){
        ...
        $data['paymentoption'] = $paymentoption;
        $data['payment_method'] = false;
        $data['title'] = 'Collective Student Fee';
        ...
        $childs = $this->session->userdata('parent_childs');
        foreach($childs as  $key_ch => $value_ch){
            $id = $value_ch['student_id'];
            $this->auth->validate_child($id);
            $student_ids[] = $id;
        ...
        $this->load->view('layout/parent/header', $data);
        $this->load->view('parent/student/getcollectivefee', $data);
        $this->load->view('layout/parent/footer', $data);
    }
